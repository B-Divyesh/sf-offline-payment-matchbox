...
for (const [path, file] of pairs) {
  ...
  const response = await fetch(`${origin}${path}?identity=repair-4`, { cache: 'no-store' });
  const live = Buffer.from(await response.arrayBuffer());
  identity.push({ path, status: response.status, sha256: hash(live), match: response.ok && hash(live) === hash(local) });
}

const home = await fetch(`${origin}/?policy=repair-4`, { cache: 'no-store' });
const asset = await fetch(`${origin}/${app.file}?policy=repair-4`, { cache: 'no-store' });
const releaseResponse = await fetch(`${origin}/release.json?policy=repair-4`, { cache: 'no-store' });
const release = await releaseResponse.json();
const expectedCommit = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
const packageMetadata = JSON.parse(await readFile('package.json', 'utf8'));
const missing = await fetch(`${origin}/not-a-real-page-repair-4`, { cache: 'no-store' });
const missingBody = Buffer.from(await missing.arrayBuffer());
const checkout = await fetch('https://api.sociobot.in/api/v1/products/offline-payment-matchbox/checkout', { redirect: 'manual' });
const invalidLicense = await fetch('https://api.sociobot.in/api/v1/products/offline-payment-matchbox/verify?license=repair-4-live-identity', { headers: { Origin: origin } });
...
const policy = {
  ...
  releaseCache: releaseResponse.headers.get('cache-control'),
  release,
  expectedCommit,
  ...
  invalidLicense: {
    status: invalidLicense.status,
    cache: invalidLicense.headers.get('cache-control'),
    cors: invalidLicense.headers.get('access-control-allow-origin'),
    body: invalidBody,
  },
};
...
const policyPass = policy.csp?.includes("frame-ancestors 'none'")
  ...
  && invalidLicense.headers.get('cache-control') === 'no-store'
  && invalidLicense.headers.get('access-control-allow-origin') === origin
  && invalidBody.valid === false;

const result = { identity, policy, policyPass };
await writeFile('.factory/evidence/repair-4-live-identity.json', `${JSON.stringify(result, null, 2)}\n`);
