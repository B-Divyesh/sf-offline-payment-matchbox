...
const before = await page.evaluate(async () => ({ caches: await caches.keys() }));
const releaseIdentityCached = await page.evaluate(async () => {
  await fetch('/release.json', { cache: 'no-store' });
  return Boolean(await caches.match('/release.json'));
});
serveUpdate = true;
...
if (process.env.SW_UPDATE_EVIDENCE) await writeFile(process.env.SW_UPDATE_EVIDENCE, `${JSON.stringify(result, null, 2)}\n`);
